...
\begin{abstract}
    ...
    The \ac{ac} has emerged as a promising approach to tackle the complexity of designing and coordinating these systems, by shifting the focus from individual devices to
    programming the global behavior of whole computational collectives. There are several state-of-the-art implementations of this paradigm, one of them being \ac{scafi}, which targets the \ac{jvm}.
    Concurrently, other implementations have been developed to bring \ac{ac} also to resource-constrained, ``thin'' devices that cannot support the \ac{jvm}, such as FCPP, which is based on
    the C++ programming language. The \ac{rufi} project aims to democratize the development of \ac{ac} applications by exploiting the Rust programming language's features of performance, safety and expressiveness to
    provide a minimal functional core for \ac{ac} that can be used on multiple platforms, including thin devices.
    In this paper, we will present the design and development of a module for the \ac{rufi} framework that will enable the distributed execution of \ac{rufi}-based aggregate programs.
\end{abstract}
