...
Above all, the RoundVM is responsible for generating the \ac{ast} and evaluate functions, and it does so with the \textit{nest} \ref{lst:nest} function, which gets executed when an aggregate program is called
...
This structure is responsible for managing the execution cycle of the aggregate programs, as well as the communication between neighbors, doing so by leveraging the other concepts defined in the module.
...
A more detailed explanation of the module's component will be given in sections \ref{subsec:behavior} and \ref{subsec:interaction}.
...
\begin{enumerate}
    ...
    \item the Device i finds the message from Device 1 with its Export, adding it to its Context for the current computation cycle.
\end{enumerate}
