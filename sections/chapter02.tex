...
In FP, a sum type represent a choice between some types. In Rust we can define Sum types by using the \textit{enum} keyword. It is also possible to perform pattern matching over a sum type, as it is shown in the listing \ref{lst:sum_types}.
...
In FP, lambda functions or anonymous functions, are functions that can be stored inside variables and passed as arguments to other functions.
